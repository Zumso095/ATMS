...
#include "Input/LomJoystickWidget.h"

#include "CommonHardwareVisibilityBorder.h"
#include "Components/Image.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(LomJoystickWidget)

#define LOCTEXT_NAMESPACE "LomJoystick"

ULomJoystickWidget::ULomJoystickWidget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SetConsumePointerInput(true);
}

FReply ULomJoystickWidget::NativeOnTouchStarted(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
...
FReply ULomJoystickWidget::NativeOnTouchMoved(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
...
FReply ULomJoystickWidget::NativeOnTouchEnded(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
	StopInputSimulation();
	return FReply::Handled().ReleaseMouseCapture();
}

void ULomJoystickWidget::NativeOnMouseLeave(const FPointerEvent& InMouseEvent)
{
	Super::NativeOnMouseLeave(InMouseEvent);
	StopInputSimulation();
}

void ULomJoystickWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
...
void ULomJoystickWidget::HandleTouchDelta(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
	...
	FVector2D MoveStickOffset = (ScreenSpacePos - ScreenSpaceStickCenter / 1.2);
	if (bNegateYAxis)
	{
...
void ULomJoystickWidget::StopInputSimulation()
{
